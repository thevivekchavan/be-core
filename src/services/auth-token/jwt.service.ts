...
export class JwtService {
  ...
  constructor(private readonly redisService: RedisService) {
    this.accessSecret = process.env.JWT_ACCESS_SECRET || randomBytes(32).toString('hex');
    this.refreshSecret = process.env.JWT_REFRESH_SECRET || randomBytes(32).toString('hex');
  ...
  async generateTokens(username: string) {
    ...
    const accessToken = jwt.sign({ username }, this.accessSecret, accessOptions);
    const refreshToken = jwt.sign({ username }, this.refreshSecret, refreshOptions);
    // Store refresh token in Redis with username as key
    const refreshExpSec = parseInt(process.env.JWT_REFRESH_EXPIRES_SECONDS || '604800', 10); // default 7 days
